    ...
    <div className="flex h-screen">
      <aside className="w-72 border-r border-[var(--border)] bg-[var(--surface)]/60 backdrop-blur-sm p-6 hidden lg:block overflow-y-auto">
        ...
        <div className="space-y-2">
          {REPOS.map((repo) => (
            <div key={repo.name} className="border border-[var(--border)] bg-[var(--surface)] rounded-lg px-3 py-2.5">
              <div className="text-sm font-medium text-[var(--text)]">{repo.name}</div>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="border-b border-[var(--border)] bg-[var(--surface)]/60 backdrop-blur-sm px-8 py-6">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-sm text-[var(--text-muted)] mt-1">Ask questions about Ali's projects, answered from the actual source code across {REPOS.length} repositories</p>
          </div>
        </header>
        ...
        <div className="border-t border-[var(--border)] bg-[var(--surface)]/60 backdrop-blur-sm px-8 py-5">
          <div className="max-w-3xl mx-auto flex gap-3">
            <input
              ...
              disabled={loading}
              className="flex-1 bg-[var(--surface)] border border-[var(--border)] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[var(--accent-blue)] disabled:opacity-50"
            />
            <button
              onClick={handleSend}
              disabled={loading}
